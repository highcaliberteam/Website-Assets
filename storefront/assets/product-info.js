    ...
    class ProductInfo extends HTMLElement {
      ...
      handleUpdateProductInfo(productUrl) {
        return (html) => {
          ...
          const updateSourceFromDestination = (
            ...
            if (source && destination) {
              ...
              blockDestinations.forEach((blockDestination) => {
                const blockSource = html.getElementById(blockDestination.id);

                if (blockSource) {
                  if (blockSource.classList.contains('hidden')) {
                    blockDestination.classList.add('hidden');
                  } else {
                    blockDestination.classList.remove('hidden');
                  }
          ...
          updateSourceFromDestination('VariantText');

          updateSourceFromDestination('ProductBreaks');
      ...
      updateMediaGrouping() {
        ...
        mediaGalleries.forEach((mediaGallery) => {
          const groupVariants = mediaGallery.getAttribute(
            'data-group-variants'
          );
          const hasOnlyDefaultVariant = mediaGallery.getAttribute(
            'data-has-only-default-variant'
          );
          const disableImageGrouping = mediaGallery.getAttribute(
          ...
          if (
            ...
            disableImageGrouping === 'true'
          ) {
            return;
          }

          const mediaItems = mediaGallery.querySelectorAll(
            '.product__media-item, .thumbnail-list__item'
          );
          const featuredMedia = Array.from(mediaItems).find(
            (media) => !media.classList.contains('hide-image')
          );
          const featuredVariantGrouping = featuredMedia
            ? featuredMedia.getAttribute('data-variant-grouping')
            : null;

          mediaItems.forEach((mediaItem) => {
            const mediaVariantGrouping = mediaItem.getAttribute(
          ...
          if (modalContent) {
            modalContent
              .querySelectorAll('.product-modal-image')
              .forEach((mediaItem) => {
                const mediaVariantGrouping = mediaItem.getAttribute(
                  'data-variant-grouping'
                );
                mediaItem.classList.toggle(
                  'hide-image',
                  mediaVariantGrouping !== featuredVariantGrouping
                );
              });
          }
        });
